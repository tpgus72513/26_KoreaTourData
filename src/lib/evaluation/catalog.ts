...
export const RESEARCH_REFERENCES = [
  ...
  { id: 'R8', title: 'Greco et al. (2019)', url: 'https://link.springer.com/article/10.1007/s11205-017-1832-9', application: '정규화·가중·집계·민감도 결과 해석 안내' },
...
export const ILLUSTRATIVE_MODEL: EvaluationModel = {
  ...
  status: 'illustrative',
  weightMethod: '동등가중치 20% · 기본 설정',
  domains: [
    { id: 'resources', label: '관광자원 기반', references: ['R1', 'R2'], indicator: '핵심 자원 운영 가능률', denominator: '핵심 관광자원 20곳' },
    { id: 'stay-local', label: '체류·지역소비 지원 여건', references: ['R2', 'R5'], indicator: '숙박·식음 운영 연결률', denominator: '핵심 관광지 20곳' },
    { id: 'mobility', label: '권역 내 이동 연결', references: ['R2'], indicator: '계획 시간 내 이동 연결률', denominator: '사전 지정 지점 쌍 20개' },
    { id: 'accessibility', label: '관광약자 이용 여건', references: ['R4', 'R9'], indicator: '지정 이용자 이용조건 충족률', denominator: '휠체어 이용자 동선·서비스 조사 항목 20개' },
    { id: 'community-environment', label: '주민·환경 여건', references: ['R3', 'R6'], indicator: '주민·환경 사전 합의조건 충족률', denominator: '주민·관리기관 합의조건 20개' },
  ].map((domain) => ({
    id: domain.id,
    label: domain.label,
    weight: 0.2,
    indicators: [{
      ...
      benchmarkRationale: '0~100% 충족률 척도를 적용한 계산 기준입니다.',
...
export function createDemoEvaluation(regionId: RegionId): EvaluationBundle {
  ...
  const input: EvaluationInput = {
    ...
    gates: [
      { id: 'continuous-route', label: '지정 이용자의 필수 동선', status: regionId === 'dosan-yekki' ? 'fail' : 'unknown', source: '현장 확인 항목' },
      { id: 'environment-operation', label: '환경 관리조건 및 운영주체 참여', status: 'unknown', source: '협의 확인 항목' },
    ],
  };
