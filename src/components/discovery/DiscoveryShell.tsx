...
import { formatPublishedAt, getRegion, getRegionName, recommendationLabel } from './discovery-utils';
  ...
  return (
    <div className="discovery-app">
      <header className="app-header">
        ...
        <div className="header-context"><strong>안동시</strong><span>분석 기준일 {publishedAt}</span></div>
      ...
      <main className="discovery-main">
        <section className="priority-banner" aria-labelledby="discovery-title">
          ...
          {selected && <div className="priority-reason"><span>현재 우선 검토</span><strong>{selectedName}</strong><p>{selected.reasons[0]}</p><Link href="/compare">분석 방법 보기</Link></div>}
        </section>
        ...
        <section className="map-workspace" aria-label="관광권역 탐색 작업 영역">
          <div className="filters" aria-label="관광 자원 필터">
            ...
            <span>데이터 확인 상태: {snapshot.mode === 'demo' ? '예시' : '확인된 데이터'}</span>
          </div>
        ...
        {selected && <aside className="region-summary" aria-labelledby="region-summary-title">
          ...
          <ScorePair potential={selected.potentialScore} confidence={selected.confidenceScore} />
          ...
          <p className="data-date">데이터 기준일 {publishedAt}</p>
        ...
        <section className="quick-compare" aria-labelledby="quick-compare-title">
          ...
          <div className="quick-compare-grid">
            {snapshot.regions.map((region) => <button key={region.id} type="button" onClick={() => setSelectedRegionId(region.id)}><strong>{getRegionName(region.id)}</strong><span>잠재력 {region.potentialScore ?? '산출 대기'}</span><span>신뢰도 {region.confidenceScore ?? '산출 대기'}</span></button>)}
          </div>
        </section>
      </main>
    </div>
