...
## 1차 제출용 실제 OpenAPI 화면

**공개 서비스:** https://donghaeng-local.vercel.app/data · 웹 · 로그인 불필요.

`/data`에서 2026-09-21에 실제 수집한 한국관광공사 관광자원 127개(원천 128개 중 미제공 좌표 1개 제외)와 2026-08-20 안동시 외지인 방문자 맥락을 확인할 수 있습니다. 자원 지도·권역별 분포·유형 필터·주소·원천 ID·수집시각을 제공하며 별도 로그인은 필요하지 않습니다. 공개 정보 사본을 배포에 포함하므로 이 화면은 DB 없이 동작합니다. 실시간 갱신이나 실제 권역별 여건 점수 산출은 제공하지 않습니다. 아래의 데모·DB 라이브 설명은 기존 업무 흐름에 해당합니다. [수집 결과·재현·한계](docs/research/public-api-capture.md)를 참고하세요.
...
- DB 환경 변수가 없으면 기존 업무 흐름은 `demo` 모드로 동작합니다. 모든 점수와 판단은 **`예시 데이터 · 정책 판단 금지`**로 표시되며, 브라우저에서의 과제·검증 변경은 세션 시연용입니다. `/data`의 실제 API 공개 사본은 별도로 유지합니다.
